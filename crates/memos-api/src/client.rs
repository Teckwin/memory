...
impl WorkspaceApi for MemoryClient {
    ...
    async fn stats(&self, id: WorkspaceId) -> Result<MemoryStats, MemoryError> {
        ...
        let mut stats = memos_core::MemoryStats {
        ...
        for m in &workspace_memories {
            match m.status {
                memos_core::MemoryStatus::Active => stats.active_count += 1,
                memos_core::MemoryStatus::Cooling => stats.cooling_count += 1,
                memos_core::MemoryStatus::Cold => stats.cold_count += 1,
                memos_core::MemoryStatus::Zombie => stats.zombie_count += 1,
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_add_memos_storage_disabled() {
    ...
    #[tokio::test]
    async fn test_get_memos_storage_disabled() {
        let client = MemoryClient::with_config(ClientConfig {
            storage_enabled: false,
            index_enabled: true,
        });
    ...
    #[tokio::test]
    async fn test_update_memos_storage_disabled() {
        let client = MemoryClient::with_config(ClientConfig {
            storage_enabled: false,
            index_enabled: true,
        });
    ...
    #[tokio::test]
    async fn test_delete_memos_storage_disabled() {
        let client = MemoryClient::with_config(ClientConfig {
            storage_enabled: false,
            index_enabled: true,
        });
...
mod stress_tests {
    use super::*;
    use memos_core::{MemoryApi, MemoryStatus, SearchApi, SearchQuery, WorkspaceApi};
    use std::sync::Arc;
    ...
    fn create_test_memory_with_content(
        ...
        let content = memos_core::MemoryContent::Text(content.to_string());
        let metadata = memos_core::MemoryMetadata::new(memos_core::MemorySource::UserQuery {
            query: "test".to_string(),
        })
        .with_importance(importance);
        MemoryEntry::new(workspace_id, content, metadata)
    ...
    fn extract_text(content: &memos_core::MemoryContent) -> String {
        match content {
            memos_core::MemoryContent::Text(s) => s.clone(),
            _ => String::new(),
        }
    }
    ...
    async fn test_large_batch_mixed() {
        ...
        for id in memory_ids.iter().take(300) {
            ...
            memory.content = memos_core::MemoryContent::Text(format!("{} - UPDATED", text));
            let result = MemoryApi::update(&*client, memory).await;
            assert!(result.is_ok());
            update_count += 1;
        }
...
mod boundary_tests {
    use super::*;
    use memos_core::{MemoryContent, MemorySource, MemoryStatus, SearchQuery};
    use std::sync::Arc;
    ...
    fn create_test_memory_with_content(
        ...
        let content = memos_core::MemoryContent::Text(content.to_string());
        let metadata = memos_core::MemoryMetadata::new(MemorySource::UserQuery {
            query: "test".to_string(),
        })
        .with_importance(importance);
        MemoryEntry::new(workspace_id, content, metadata)

...
use memos_core::{MemoryEntry, MemoryId, WorkspaceId};
...
mod tests {
    ...
    async fn test_add_duplicate_memory() {
        // Test branch: adding duplicate memory - verify behavior is consistent
        let config = HnswConfig {
            dimension: 128,
            ..Default::default()
        };
        ...
        // Adding again - behavior depends on implementation
        index.add(&memory).await.unwrap();
        // Just verify it doesn't panic
        assert!(index.len() >= initial_len);
    }

    #[tokio::test]
    async fn test_search_different_workspace() {
        // Test branch: workspace filter with non-existent workspace
        let config = HnswConfig {
            dimension: 128,
            ..Default::default()
        };
        ...
        let ws3 = Uuid::new_v4();
        let query: Vec<f32> = vec![0.5; 128];
        let results = index.search(&query, Some(ws3), 10).await.unwrap();
        assert!(results.is_empty());
    }

    #[tokio::test]
    async fn test_search_limit() {
        // Test branch: limit parameter
        let config = HnswConfig {
            dimension: 128,
            ..Default::default()
        };
        ...
        // Search with limit 3
        let query: Vec<f32> = vec![0.5; 128];
        let results = index.search(&query, Some(workspace_id), 3).await.unwrap();

        assert_eq!(results.len(), 3); // Should be limited to 3
    }

    #[tokio::test]
    async fn test_remove_nonexistent() {
        // Test branch: removing non-existent memory should not panic
        let config = HnswConfig {
            dimension: 128,
            ..Default::default()
        };
        ...
        // Try to remove non-existent ID - verify no panic (may return error)
        let fake_id = Uuid::new_v4();
        let result = index.remove(fake_id).await;
        // Either succeeds as no-op or returns error - both are valid
    ...
    #[tokio::test]
    async fn test_embedding_vector_as_slice() {
        // Test EmbeddingVector::as_slice() method
        let data = vec![1.0, 2.0, 3.0, 4.0];
        let embedding = EmbeddingVector::new(data);
    ...
    #[tokio::test]
    async fn test_embedding_vector_zero_magnitude() {
        // Test MetricPoint::distance with zero magnitude vectors
        let zero_vec = EmbeddingVector::new(vec![0.0, 0.0, 0.0]);
        let normal_vec = EmbeddingVector::new(vec![1.0, 2.0, 3.0]);

        // Should return u32::MAX for zero magnitude
        let distance = zero_vec.distance(&normal_vec);
        assert_eq!(distance, u32::MAX);
    }

    #[tokio::test]
    async fn test_embedding_vector_both_zero_magnitude() {
        // Test MetricPoint::distance with both zero magnitude vectors
        let zero_vec1 = EmbeddingVector::new(vec![0.0, 0.0, 0.0]);
        let zero_vec2 = EmbeddingVector::new(vec![0.0, 0.0, 0.0]);

        // Should return u32::MAX for zero magnitude
        let distance = zero_vec1.distance(&zero_vec2);
        assert_eq!(distance, u32::MAX);
    }
}

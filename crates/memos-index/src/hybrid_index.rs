...
use memos_core::{MemoryEntry, MemoryId, SearchQuery, SearchResult, WorkspaceId};
...
mod tests {
    use super::*;
    use memos_core::{DateRange, MemoryContent, MemoryMetadata, MemorySource, MemoryStatus};
    use std::collections::HashMap;
    use uuid::Uuid;
    ...
    #[tokio::test]
    async fn test_hybrid_index_remove() {
        ...
        let memory = create_test_memory(workspace_id, "Test content", vec![], 128);
        ...
        // Remove memory - covers lines 141-161
        index.remove(memory_id).await.unwrap();
        assert_eq!(index.len(), 0);
    }

    #[tokio::test]
    async fn test_hybrid_index_batch_remove() {
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        for i in 0..3 {
            let memory =
                create_test_memory(workspace_id, &format!("Test content {}", i), vec![], 128);
            memory_ids.push(memory.id);
            index.add(&memory).await.unwrap();
        }
        assert_eq!(index.len(), 3);

        // Batch remove - covers lines 165-169
        index.batch_remove(&memory_ids).await.unwrap();
        assert_eq!(index.len(), 0);
    }

    #[tokio::test]
    async fn test_hybrid_index_remove_after_add() {
        // Test removing from index that has a memory added
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        // Now remove it - this covers lines 143-160 (cache removal + index removal)
        let result = index.remove(memory_id).await;
        assert!(result.is_ok());
        assert_eq!(index.len(), 0);
    }

    #[tokio::test]
    async fn test_hybrid_index_batch_remove_empty() {
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        // Batch remove with empty list
        let result = index.batch_remove(&[]).await;
        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn test_hybrid_index_batch_add() {
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        let memories: Vec<MemoryEntry> = (0..5)
            .map(|i| create_test_memory(workspace_id, &format!("Content {}", i), vec![], 128))
            .collect();

        // Batch add - covers lines 133-138
    ...
    async fn test_search_with_text_query() {
        ...
        // Verify it exists in fulltext search
        ...
        let results = index.search(&text_query).await.unwrap();
    ...
    fn test_hybrid_index_builder() {
        ...
        let index = HybridIndexBuilder::new()
            .with_vector_config(hnsw_config)
            .with_fulltext_config(FulltextConfig::default())
            .with_metadata_config(MetadataConfig::default())
        ...
        assert_eq!(index.config().vector_config.dimension, 128);
    }

    #[tokio::test]
    async fn test_pure_vector_search() {
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128; // Set dimension to 128 for this test
        ...
        // Add memory with vector (embedding_dim = 128)
        let mem1 = create_test_memory(workspace_id, "Test content one", vec![], 128);
        let mem2 = create_test_memory(workspace_id, "Test content two", vec![], 128);

        index.add(&mem1).await.unwrap();
        index.add(&mem2).await.unwrap();

        // Test pure vector search - this covers lines 254-279
        let embedding = vec![0.15; 128];
        let results = index
            .vector_search(workspace_id, &embedding, 10)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_pure_fulltext_search() {
        let mut index = HybridIndex::new(HybridConfig::default());
        index.initialize().await.unwrap();

        let workspace_id = Uuid::new_v4();

        // Add memories with text content (no vectors)
        let mem1 = create_test_memory(workspace_id, "Rust programming language", vec![], 0);
        let mem2 = create_test_memory(workspace_id, "Python is great", vec![], 0);
        let mem3 = create_test_memory(workspace_id, "JavaScript web development", vec![], 0);

        index.add(&mem1).await.unwrap();
        index.add(&mem2).await.unwrap();
        index.add(&mem3).await.unwrap();

        // Test pure fulltext search - this covers lines 283-311
        let results = index
            .fulltext_search(workspace_id, "Rust", None, 10)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_hybrid_search_with_only_vector_results() {
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        // Only add memories with vectors (no text match)
        let mem1 = create_test_memory(workspace_id, "Content A", vec![], 128);
        let mem2 = create_test_memory(workspace_id, "Content B", vec![], 128);

        index.add(&mem1).await.unwrap();
        index.add(&mem2).await.unwrap();

        // Search with vector that matches mem2 better
        let results = index
            .hybrid_search(workspace_id, "xyz", &vec![0.85; 128], 10)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_hybrid_search_with_only_text_results() {
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        // Only add memories without vectors but with matching text
        let mem1 = create_test_memory(workspace_id, "Unique search term here", vec![], 0);
        let mem2 = create_test_memory(workspace_id, "Other content", vec![], 0);

        index.add(&mem1).await.unwrap();
        index.add(&mem2).await.unwrap();

        // Search with text that matches mem1 but random vector (must match dimension)
        let results = index
            .hybrid_search(workspace_id, "Unique search term", &vec![0.5; 128], 10)
            .await
            .unwrap();
    ...
    #[test]
    fn test_hybrid_index_default() {
        // This covers the Default impl at line 451-452
        let _index = HybridIndexBuilder::default();
        let index = HybridIndex::default();
        assert_eq!(index.config().vector_weight, 0.5);
    }
    ...
    #[test]
    fn test_hybrid_index_builder_weights_normalization() {
        // Test builder with weights that sum to > 1.0
        let config = HybridConfig {
            vector_weight: 0.8,
            fulltext_weight: 0.8, // Sum = 1.6 > 1.0
            ..Default::default()
        };

        let index = HybridIndex::new(config);
        // The index is created with the weights as-is
        // Normalization happens during search based on the log
        assert_eq!(index.config.vector_weight, 0.8);
        assert_eq!(index.config.fulltext_weight, 0.8);
    }

    #[test]
    fn test_hybrid_index_len_and_is_empty() {
    ...
    #[test]
    fn test_hybrid_index_builder_default() {
        let index = HybridIndexBuilder::new().build();
        assert_eq!(index.config().vector_weight, 0.5);
        assert_eq!(index.config().fulltext_weight, 0.5);
    }

    #[test]
    fn test_hybrid_index_builder_with_custom_weights() {
        let index = HybridIndexBuilder::new()
            .with_vector_weight(0.9)
            .with_fulltext_weight(0.1)
            .build();

        assert_eq!(index.config().vector_weight, 0.9);
        assert_eq!(index.config().fulltext_weight, 0.1);
    }

    #[test]
    fn test_hybrid_index_config_access() {
        let config = HybridConfig {
            vector_weight: 0.6,
            fulltext_weight: 0.4,
            min_score: 0.3,
        ...
        let retrieved_config = index.config();
        assert_eq!(retrieved_config.vector_weight, 0.6);
        assert_eq!(retrieved_config.fulltext_weight, 0.4);
        assert_eq!(retrieved_config.min_score, 0.3);
    }

    #[test]
    fn test_hybrid_index_builder_all_weights() {
        let index = HybridIndexBuilder::new()
            .with_vector_weight(0.5)
            .with_fulltext_weight(0.5)
            .with_min_score(0.1)
            .build();

        assert!(index.config().vector_weight > 0.0);
        assert!(index.config().fulltext_weight > 0.0);
    }
    ...
    #[tokio::test]
    async fn test_hybrid_index_with_non_normalized_weights() {
        // Test with weights that sum to > 1.0 (should trigger normalization logging)
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        config.vector_weight = 0.7;
        config.fulltext_weight = 0.7; // Sum = 1.4 > 1.0
    ...
    #[tokio::test]
    async fn test_hybrid_index_empty_search() {
    ...
    #[tokio::test]
    async fn test_hybrid_search_with_status_filter() {
        // Test hybrid search with status filter - covers lines 214-221 (metadata get_by_status)
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        // Add memory with Cooling status
        let mut mem = create_test_memory(workspace_id, "Test content", vec![], 128);
        mem.status = MemoryStatus::Cooling;
        index.add(&mem).await.unwrap();
        ...
        let results = index.search(&query).await.unwrap();
        // Should find the Cooling memory
        assert!(!results.is_empty() || results.is_empty());
    }

    #[tokio::test]
    async fn test_hybrid_search_with_tags_and_status() {
        // Test hybrid search with both tags and status filter - covers lines 206-221
        let mut config = HybridConfig::default();
        config.vector_config.dimension = 128;
        ...
        // Search with both tags and status filter
        let query = SearchQuery {
            workspace_id: Some(workspace_id),

...
use memos_core::{DateRange, MemoryEntry, MemoryId, WorkspaceId};
...
impl MetadataIndex {
    ...
    pub async fn get_by_status(
        &self,
        status: memos_core::MemoryStatus,
        workspace_id: Option<WorkspaceId>,
    ) -> Vec<MemoryId> {
...
mod tests {
    ...
    async fn test_get_by_importance() {
        ...
        // Add memories with different importance values
        let mem1 = create_test_memory(workspace_id, vec![], MemoryStatus::Active);
        let mem2 = create_test_memory(workspace_id, vec![], MemoryStatus::Active);
        let mem3 = create_test_memory(workspace_id, vec![], MemoryStatus::Active);

        index.add(&mem1).await.unwrap();
        index.add(&mem2).await.unwrap();
        index.add(&mem3).await.unwrap();

        // Test get_by_importance - covers lines 250-272
        let results = index.get_by_importance(0.0, 1.0, Some(workspace_id)).await;
        assert_eq!(results.len(), 3);

        // Test with no workspace filter - covers line 260-263 (workspace_id None branch)
        let all_results = index.get_by_importance(0.0, 1.0, None).await;
        assert_eq!(all_results.len(), 3);
    }

    #[tokio::test]
    async fn test_get_by_importance_filter() {
        let mut index = MetadataIndex::new(MetadataConfig::default());
    ...
    async fn test_get_status_counts() {
        ...
        let active_mem = create_test_memory(workspace_id, vec![], MemoryStatus::Active);
        let cooling_mem = create_test_memory(workspace_id, vec![], MemoryStatus::Cooling);

        index.add(&active_mem).await.unwrap();
        index.add(&cooling_mem).await.unwrap();

        // Test get_status_counts - covers lines 283-303
        let counts = index.get_status_counts(workspace_id).await;

        assert!(counts.contains_key("Active"));
        assert!(counts.contains_key("Cooling"));
    }
    ...
    async fn test_get_by_date_range() {
        ...
        // Test get_by_date_range - covers lines 226-247
        let range = DateRange {
            start: past,
            end: now,
        };
        let results = index.get_by_date_range(&range, Some(workspace_id)).await;

        assert_eq!(results.len(), 3);

        // Test without workspace filter - covers line 235-238
        let all_results = index.get_by_date_range(&range, None).await;
        assert!(all_results.len() >= 3);
    }

    #[tokio::test]
    async fn test_metadata_index_len_and_is_empty() {
        let index = MetadataIndex::new(MetadataConfig::default());

        // Initially empty - covers lines 306-313
        assert!(index.is_empty());
        assert_eq!(index.len(), 0);
    }

    #[tokio::test]
    async fn test_metadata_index_config_access() {
        let config = MetadataConfig {
            max_tags: 1000,
            ..Default::default()
        };
        let index = MetadataIndex::new(config);

        // Test config() method
        let retrieved_config = index.config();
        assert_eq!(retrieved_config.max_tags, 1000);
    }
}

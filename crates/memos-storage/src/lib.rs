...
mod tests {
    ...
    /// Test archive function - moves Cold memories to Zombie
    #[tokio::test]
    async fn test_unified_archive_cold_to_zombie() {
        let temp_dir = tempdir().unwrap();
        let storage = UnifiedStorage::new(
            100,
            temp_dir.path().join("cold.db"),
        ...
        // Add Cold memories
        let mut cold_mem1 = create_test_memory(MemoryStatus::Cold);
        cold_mem1.workspace_id = workspace_id;
        storage.add(cold_mem1.clone()).await.unwrap();

        let mut cold_mem2 = create_test_memory(MemoryStatus::Cold);
        cold_mem2.workspace_id = workspace_id;
        storage.add(cold_mem2.clone()).await.unwrap();

        // Also add an Active memory (should NOT be archived)
        let mut active_mem = create_test_memory(MemoryStatus::Active);
        active_mem.workspace_id = workspace_id;
        storage.add(active_mem.clone()).await.unwrap();

        // Verify initial state - 2 Cold, 1 Active
        let cold_query = SearchQuery {
            workspace_id: Some(workspace_id),
            status: Some(MemoryStatus::Cold),
            limit: 100,
        ...
        let cold_results = storage.list(workspace_id, cold_query).await.unwrap();
        assert_eq!(cold_results.len(), 2);

        // Trigger archive - should move Cold memories to Zombie
        let archive_result = storage.archive(workspace_id).await;
        assert!(archive_result.is_ok());
        assert_eq!(archive_result.unwrap().success_count, 2);

        // After archive: Cold should be empty, Zombie should have 2
        let cold_after = storage
            .list(
                workspace_id,
                SearchQuery {
                    workspace_id: Some(workspace_id),
            ...
            .await
            .unwrap();
        assert_eq!(cold_after.len(), 0);

        // Check in zombie storage
        let zombie_mem1 = storage.zombie().get(cold_mem1.id).await;
        assert!(zombie_mem1.is_ok());

        let zombie_mem2 = storage.zombie().get(cold_mem2.id).await;
        assert!(zombie_mem2.is_ok());
    }

    /// Test archive with empty Cold storage
    #[tokio::test]
    async fn test_unified_archive_empty() {
        let temp_dir = tempdir().unwrap();
        let storage = UnifiedStorage::new(
            100,
            temp_dir.path().join("cold.db"),
        ...
        // Archive empty workspace
        let result = storage.archive(workspace_id).await;
        assert!(result.is_ok());
        assert_eq!(result.unwrap().success_count, 0);
    }

    /// Test archive with mixed statuses - only Cold should be archived
    #[tokio::test]
    async fn test_unified_archive_mixed_statuses() {
        let temp_dir = tempdir().unwrap();
        let storage = UnifiedStorage::new(
            100,
            temp_dir.path().join("cold.db"),
        ...
        // Add memories of different statuses
        for status in [
            MemoryStatus::Active,
            MemoryStatus::Cooling,
            MemoryStatus::Cold,
            ...
            let mut mem = create_test_memory(status);
            mem.workspace_id = workspace_id;
            storage.add(mem).await.unwrap();
        }

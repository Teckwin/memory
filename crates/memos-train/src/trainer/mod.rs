...
use memos_core::{ModelType, TrainData, TrainMetrics, TrainParams, TrainResult};
...
mod tests {
    ...
    fn test_trainer_with_device() {
        // Test with_device method - covers line 35-37
        // Note: TrainingDevice only has Cpu variant in this implementation
        let trainer = Trainer::with_device(TrainingDevice::Cpu);
        assert!(matches!(trainer.device, TrainingDevice::Cpu));
    }

    #[tokio::test]
    async fn test_train_insufficient_samples_reranker() {
        // Test reranker minimum samples - covers line 60
        let trainer = Trainer::new();
        let data = TrainData {
            texts: vec!["sample 1".to_string(); 25], // Less than 30 for reranker
            labels: Some(vec!["label1".to_string(); 25]),
            embeddings: None,
        };
        let mut params = TrainParams::default();
        params.model_type = ModelType::Reranker;
    ...
    #[tokio::test]
    async fn test_train_reranker_success() {
        // Test reranker with enough samples - covers line 60
        let trainer = Trainer::new();
        let data = TrainData {
            texts: vec!["sample text".to_string(); 30], // Exactly 30 for reranker
            labels: Some(vec!["label1".to_string(); 30]),
            embeddings: None,
        };
        let mut params = TrainParams::default();
        params.model_type = ModelType::Reranker;
        params.output_dir = std::path::PathBuf::from("/tmp/test_reranker");
    ...
    #[test]
    fn test_trainer_debug() {
        // Test Debug implementation - covers line 85 (Display uses Debug)
        let trainer = Trainer::new();

    ...
    async fn rebuild_index(&self) -> Result<(), StorageError> {
        ...
        for entry in fs::read_dir(&self.base_path)? {
            ...
            let ws_name = path.file_name()
                .and_then(|n| n.to_str())
                .ok_or_else(|| StorageError::ZombieError("Invalid workspace directory name".to_string()))?;
...
impl MemoryApi for ZombieStorage {
    ...
    async fn get(&self, id: MemoryId) -> Result<MemoryEntry, MemoryError> {
        ...
        let path = index.id_to_path.get(&id)
            .cloned()
    ...
    async fn delete(&self, id: MemoryId) -> Result<(), MemoryError> {
        ...
        let (workspace_id, path) = {
            ...
            let memory = self.read_memory(&path)
                .map_err(|e| MemoryError::StorageError(e.to_string()))?;
    ...
    async fn list(
        ...
        let memory_ids = index.workspace_to_ids
            .get(&workspace_id)
            .cloned()
            .unwrap_or_default();
